...
```
  ...
  * `which` helps to define which layer to replace and the default setting is 1 (the parent layer `rasterly()`).
...
#### `plotly` graphics
  ...
  + `add_rasterly()`: "Plotly" pipes `add_trace(...)` to layer up. `rasterly` builds `add_rasterly()` from `add_heatmap()` to enable its functionality in "plotly"
    ...
    ```{r add_rasterly, fig.width = 4, fig.height = 3}
  ...
  + `plotly.rasterly()`: `plotly.rasterly` takes a `rasterly` object and returns a "plotly" object
    ...
    ```{r plotly_rasterly, warning=FALSE, message=FALSE, fig.width = 4, fig.height = 3}
    # plotly
    ply <- p %>% 
