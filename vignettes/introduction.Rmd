...
`rasterizer()` is a parent layer for initial settings, which includes `plot_height`, `plot_width` and others for generating rasters; then, children layers such as `rasterize_points()` can be piped into. Note that, "p" is a list of environments.
...
p["background"]
# background in children layer `rasterizer_points()` is replaced
...
#### `plotly` graphics
  ...
  + `plotly.rasterizer()`: `plotly.rasterizer` takes a `rasterizer` object and returns a "plotly" object
    ...
    # plotly
    ply <- p %>% 
      plotly.rasterizer(sizing = "contain")
    ply
    ```

## API

`rasterizer` application programming interface
