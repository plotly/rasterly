#' @export
raster_2D <- function(M, colour_map = c('lightblue','darkblue'), span = 50,
...
#' @export
raster_3D <- function(L, colour_key = NULL, layout = c("weighted", "cover"), span = 50,
                      zeroIgnored = TRUE, image = NULL, background = "#FFFFFF",
                      alpha = 255) {
  ...
  if(layout == "weighted") {
    ...
    lapply(1:n,
           function(i){
             image <<- raster_2D(M = L[[i]], colour_map = c(background, colour_key[i]),
                                 span = span/10,
