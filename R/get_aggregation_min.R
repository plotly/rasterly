get_aggregation.min <- function(plot_width, plot_height, aesthetics,
  ...
  L <- if(group_by_data_table) {
    ...
    display <- aesthetics[,
                          list(
                            display = list(
                              aggregation_minCpp(plot_width = plot_width, plot_height = plot_height,
    ...
    if(is_colour) {
      ...
      agg_minCpp(L = lapply(1:length(levels), function(i) matrix(0, nrow = plot_height, ncol = plot_width)),
                 levels = levels,
